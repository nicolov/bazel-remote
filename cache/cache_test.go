...
func TestCacheZeroFile(t *testing.T) {
	...
	four := []byte{0, 0, 0, 0}
	const fourSha256 = "df3f619804a92fdb4057192dc43dd748ea778adc52bc498ce80524c014b81119"

	err := cache.Put(KEY, int64(4), fourSha256, bytes.NewReader(four))
	if err == nil {
		t.Fatal("expected rejected zeros file")
	}
}

func TestZeroCheckWriter(t *testing.T) {
	checkIsAllZeros := func(seq []byte, expectedNotAllZeros bool) {
		checker := &ZeroCheckWriter{}
		_, err := io.Copy(checker, bytes.NewReader(seq))
		if err != nil {
			t.Fatal(err)
		}
		if checker.hasNoZeros != expectedNotAllZeros {
			t.Fatalf("expected notAllZeros to be %s, found %s", expectedNotAllZeros, checker.hasNoZeros)
		}
	}

	checkIsAllZeros([]byte{0, 0, 0}, false)
	checkIsAllZeros([]byte{0, 1, 0}, true)
}
